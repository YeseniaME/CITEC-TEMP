...
abstract class CCodeModel extends CFormModel
{
	...
	public function validateTemplate($attribute,$params)
	{
		...
		if(!isset($templates[$this->template]))
			$this->addError('template', 'Selección de plantilla inválida.');
		else
		{
			...
			foreach($this->requiredTemplates() as $template)
			{
				if(!is_file($templatePath.'/'.$template))
					$this->addError('template', "No es posible encontrar el archivo de la plantilla '$template'.");
			}
		}
	}
	...
	{
		return 'El código se ha generado con éxito.';
	}
	...
	{
		return 'Hubo algunos errores al genera el código. Por favor, consulte los siguientes mensajes.';
	}
	...
	public function getTemplatePath()
	{
		...
		elseif(empty($templates))
			throw new CHttpException(500,'No hay plantillas disponibles.');
		else
			throw new CHttpException(500,'La selección de plantilla no es válida.');
	...
	public function render($templateFile,$_params_=null)
	{
		if(!is_file($templateFile))
			throw new CException("El archivo de plantilla '$templateFile' no existe.");
	...
	{
		$output='Generación de código usando la plantilla "'.$this->templatePath."\"...\n";
		foreach($this->files as $file)
		{
			if($file->error!==null)
