	...
	public function save()
	{
		...
		if($this->content===null)  // a directory
		{
			if(!is_dir($this->path))
			{
				...
				if(!$result)
				{
					$this->error="No se puede crear el directorio '{$this->path}'.";
					return false;
				}
			}
			return true;
		...
		if($this->operation===self::OP_NEW)
		{
			...
			if(!is_dir($dir))
			{
				...
				if(!$result)
				{
					$this->error="No se puede crear el directorio '$dir'.";
					return false;
				}
			}
		}
		...
		{
			$this->error="No se puede escribir el archivo '{$this->path}'.";
			return false;
		}
		else
		{
